...
class YPLibraryVC: UIViewController, YPPermissionCheckable {
    ...
    internal var selectionIndexPath = IndexPath(row: 0, section: 0)
...
//        if multipleSelectionEnabled {
//            // Modified by Dom
//            // Disable auto select on current image
...
//    }
    ...
    func checkPermissionToAccessPhotoLibrary(block: @escaping (Bool) -> Void) {
        ...
        case .authorized:
            ...
            if photoCount > 0 { //have image selected in gallery
                block(true)
            }else {
                // not yet have image selected in gallery need to change permission
                customOption(type       : .MultipleChoice,
                             image      : "",
                             title      : "allow_photos",
                             message    : "select_more_images_or_enable_library_access",
                             data       : ["select_more_mages", "enable_library_access"],
                             selectRowAt: -1,
                             imgStrRec  : ["", ""],
                             shouldDimiss: false) { (index) in
                    switch index {
                    
                    case -1: //DidTapCloseBtn, close presenting vc
                        self.presentingViewController?.popOrDismissVC()
                        
                    case 0:
                        
                        // Show library picker
                        if #available(iOS 14, *) {
//                            MyNotify.send(name: .presentLimitedLibraryPicker)
                        } else {
                            // Fallback on earlier versions
                        }
                        
                    case 1  : // grant Permission go to photo setting app
        ...
        case .restricted, .denied:
            ...
            customOption(type       : .MultipleChoice,
                         image      : "",
                         title      : "allow_photos",
                         message    : "this_allows_WABOOKS_to_acess_your_gallery_for_creating_a_receipt",
                         data       : ["enable_library_access"],
                         selectRowAt: -1,
                         imgStrRec  : [""],
                         shouldDimiss: true) { (index) in
                switch index {
                
                case -1: //DidTapCloseBtn, close presenting vc
                    self.presentingViewController?.popOrDismissVC()
                    
                case 0: // grant Permission go to photo setting app
